...
from NIHARIKACHATBOT import NIHARIKACHATBOT as app, save_clonebot_owner, save_idclonebot_owner
from NIHARIKACHATBOT import NIHARIKACHATBOT, db as mongodb
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                no_updates=False,
                plugins=dict(root="NIHARIKACHATBOT.idchatbot"),
            )
            await ai.start()
...
async def delete_cloned_session(client, message):
    try:
        ...
        if cloned_session:
            ...
            await ok.edit_text(
                f"**Your String Session has been removed from my database ✅.**\n\n**Your bot will off after restart @{NIHARIKACHATBOT.username}**"
            )
        else:
