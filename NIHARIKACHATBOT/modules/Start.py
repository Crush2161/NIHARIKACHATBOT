...
import psutil
import config
from NIHARIKACHATBOT import _boot_
from NIHARIKACHATBOT import get_readable_time
from NIHARIKACHATBOT import NIHARIKACHATBOT, mongo
...
from NIHARIKACHATBOT.database.chats import get_served_chats, add_served_chat
from NIHARIKACHATBOT.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from NIHARIKACHATBOT.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
BOT = "https://envs.sh/6Hl.jpg"
IMG = [
    "https://envs.sh/QGP.jpg",
    "https://envs.sh/QGw.jpg",
    "https://envs.sh/QGq.jpg",
    "https://envs.sh/QG0.jpg",
    "https://envs.sh/QGS.jpg",
    "https://envs.sh/QGz.jpg",
    "https://envs.sh/QG3.jpg",
    "https://envs.sh/QGY.jpg",
    "https://envs.sh/QGC.jpg",
    "https://envs.sh/QGO.jpg",
    "https://envs.sh/QGm.jpg",
    "https://envs.sh/QGM.jpg",
    "https://envs.sh/QGy.jpg",
    "https://envs.sh/QGX.jpg",
]



from NIHARIKACHATBOT import db
...
@NIHARIKACHATBOT.on_message(filters.new_chat_members)
async def welcomejej(client, message: Message):
    ...
    try:
        for member in message.new_chat_members:
            ...
            if member.id == NIHARIKACHATBOT.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await NIHARIKACHATBOT.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await NIHARIKACHATBOT.download_media(
                        chat.photo.big_file_id, file_name=f"chatpp{chat.id}.png"
                    )
                    chat_photo = (
                        groups_photo if groups_photo else "https://envs.sh/6Hl.jpg"
                    )
                except AttributeError:
                    chat_photo = "https://envs.sh/6Hl.jpg"
                except Exception as e:
                    pass

                count = await NIHARIKACHATBOT.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
                ...
                try:
                    ...
                    if OWNER:
                        await NIHARIKACHATBOT.send_photo(
                            int(OWNER_ID),
                            photo=chat_photo,
                            caption=msg,
...
@NIHARIKACHATBOT.on_cmd(["start", "aistart"])
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await NIHARIKACHATBOT.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(NIHARIKACHATBOT.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await NIHARIKACHATBOT.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
...
@NIHARIKACHATBOT.on_cmd("help")
async def help(client: NIHARIKACHATBOT, m: Message):
    if m.chat.type == ChatType.PRIVATE:
...
async def ping(_, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{NIHARIKACHATBOT.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve  αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||** ⋆ʟᴏᴠᴇ ᴡɪᴛʜ⋆ [ 𝐅𝐨𝐫𝐞𝐯𝐞𝐫 𝐂𝐫𝐮𝐬𝐡  🤎](https://t.me/{OWNER_USERNAME}) **||</b>",
...
@NIHARIKACHATBOT.on_message(filters.command("stats"))
...
from NIHARIKACHATBOT import NIHARIKACHATBOT


@NIHARIKACHATBOT.on_cmd("id")
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await NIHARIKACHATBOT.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await NIHARIKACHATBOT.send_message(
