...
from NIHARIKACHATBOT import CLONE_OWNERS
from NIHARIKACHATBOT import NIHARIKACHATBOT as app, save_clonebot_owner
from NIHARIKACHATBOT import db as mongodb, NIHARIKACHATBOT
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="NIHARIKACHATBOT/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
